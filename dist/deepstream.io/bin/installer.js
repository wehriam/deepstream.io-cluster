...
const extract = function (data) {
    ...
    try {
        if (platform === 'linux') {
            child_process_1.execSync(`mkdir -p ${outPath} && tar -xzf ${archivePath} -C ${outPath}`);
        }
        else {
    ...
    if (!process.env.QUIET) {
        console.log(chalk_1.default.green(`${data.name} ${data.version} was installed to ${outputParent}`));
    }
    return outPath;
};
/**
 ...
 * @return {void}
 */
exports.installer = (opts, callback) => {
